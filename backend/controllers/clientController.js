...
const register = (req, res) => {
    ...
    Client.find({
        ...
        if (error) return res.status(500).json({ message: "Error en la consulta de clientes" });
        ...
        newClient.save((error, clientStored) => {
            if (error || !clientStored) return res.status(400).send({ msg: "Error al crear el cliente" })
...
const listMachine = (req, res) => {
    ...
    Machine.find({ client: clientId })
        .paginate(page, itemPerPage, async (error, machines, total) => {
...
const update = (req, res) => {
    ...
    if (clientData.email) {
        Client.find({ email: clientData.email.toLowerCase() }).exec(async (error, clientes) => {
            if (error) return res.status(500).json({ message: "Error en la consulta de clientes" });
            ...
            Client.findByIdAndUpdate({ _id: id }, clientData, { new: true }, (error, clientData) => {
